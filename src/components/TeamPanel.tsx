  ...
  return (
    <div className={`flex flex-col items-center ${vertical ? 'w-full' : 'w-1/3'} p-4 ${team === 'Blue' ? 'bg-black' : 'bg-gray-900'} rounded-lg`}>
      ...
      <div className="flex items-center mb-4">
        <img src={coach.avatarUrl} alt={coach.name} className="w-12 h-12 rounded-full border-2 border-white mr-2" />
        <div>
        ...
        {picks.map((hero, idx) => (
          <div key={idx} className="flex items-center mb-2 w-full">
            <div className="relative w-16 h-16 mr-2">
              {hero ? (
                <>
                  <img src={hero.imageUrl} alt={hero.name} className="w-16 h-16 rounded-lg border-2 border-white" />
      ...
      <div className="flex space-x-2">
        {bans.map((hero, idx) => (
          <div key={idx} className="w-10 h-10 relative">
            {hero ? (
              <img src={hero.imageUrl} alt={hero.name} className="w-10 h-10 rounded bg-gray-900 border-2 border-gray-400 opacity-60" />
            ) : (
